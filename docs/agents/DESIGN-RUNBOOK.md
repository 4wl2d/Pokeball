...
- one current typed `Pulse` and a sparse `DecisionContext` containing only observations that affect the Decision;
...
ObservedSignal
ControlPulse
Projection / Reply / Effect / Signal / Timer
```
...
- pre-acceptance validation/admission/decision rejection is a `BoundaryResponse`; and
