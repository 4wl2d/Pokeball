...
For each command path, the target owns one closed command/result mapping and fixed refusal meaning. An immediate same-build typed call can use `CounterCommands.increment(): IncrementResult`, returning `Changed(value)` only after target acceptance or `NotAccepted(reason)` before acceptance. Invoke the target only from already accepted source output, outside pure `decide`, and apply completion through the source's serialized handler. The typed interface, trusted construction, and call scope establish origin and correlation; no handle, source/result token, issuer type, protocol identifier, carrier class, or tuple-reconstruction proof is required. Add an allowed consumer through wiring; caller-dependent business policy remains with the target owner.

When detached, reordered, retryable, recoverable, or independently observed delivery requires portable evidence, preserve:
...
The trusted target boundary constructs the command Pulse only from a verified accepted source frame; target `decide` is the sole acceptance point. Only an accepted target Decision creates the result output, and the verified return route derives `resultSource` from that accepted target frame. `semanticHandle` equality is correlation, not re-export or ownership transfer. Within the idempotency horizon, duplicate-before-result returns only verified ACK proof of the existing accepted target frame and pending-result state; duplicate-after-result redelivers the exact accepted result frame. Neither invokes another Decision, increments revision, fabricates a result, or repeats a Resource action.

A statically finite synchronous workflow uses its complete execution structure as its bound, including result/refusal/failure handlers. It requires no level-1/level-2 reservations or separate fan-out sum. An import DAG alone is insufficient when a result handler can issue more work. Growing or retained work still needs finite capacity and admission that cannot lose accepted completion; handoff and retry preserve any applicable total causal budget.
...
- accepted target outcome reaches the source's serialized result input through an immediate typed return or verified portable `ModuleResultPulse`;
- post-commit mechanical observation becomes a typed `ControlPulse` only if it affects Sovereign State;
- pre-acceptance validation/admission/Decision rejection is a `BoundaryResponse`; for an immediate command it may return as `NotAccepted(reason)`; independent delivery uses verified `CommandRejectedBeforeAcceptance`, never an accepted Reply or result;
...
| admission before acceptance | `BoundaryResponse(AdmissionFailure(reason))`; typed pre-acceptance return for an immediate call, verified carrier for independent delivery |
| target Decision rejection before acceptance | operation-specific pre-acceptance return, or `DecisionRejected(BusinessRejection)` inside a verified portable carrier |
| accepted target business outcome, including rejection | accepted result through typed return or portable `ModuleResultOutput`/`ModuleResultPulse` |
...
If outputs exist, prove complete-batch acceptance and commit-before-dispatch. When numeric `maxOutputBytesPerDecision` is present, the `DecisionOutputs` dimension of the binding's immutable `BoundedByteMeasure` covers the complete ordered `Decision.outputs` semantic representation. Count sequence structure, every required output-envelope field, correlation token, `sourceOrdinal`, and payload; exclude `nextState`, accepted-frame fields outside an output envelope, and later transport framing, compression, encryption, retry headers, and `AttemptId`. Add a stable `SemanticHandle` only when work outlives the call, can complete later, retry, reorder, be independently cancelled, recovered, reconciled, or observed. Immediate call-scope output uses accepted sequence position and typed call scope; temporary retention and crossing a Ball alone add no handle.
