...
[Исполняемый учебный пример](../../examples/local_composition.py) использует один `SerialOwner` для Counter и источника. Его путь принятия в псевдокоде:
...
[Поведенческие тесты](../../tests/test_local_composition.py) различают три пути:
...
Запустите самостоятельные тесты стандартной библиотекой:

```sh
python3 -m unittest discover -s tests -p 'test_local_composition.py' -v
```

Тесты выполняют следующие изменения и отдельно учитывают их стоимость:
...
| Добавить разрешённого потребителя Counter | Нет | Одно соединение `make_source(app.commands)` | Нет | Нет |
| Переименовать переменную и извлечь проверку лимита | Одно решение с переименованием и одна чистая функция | Нет | Нет | Нет |

Здесь считаются изменённые функции и соединения, а не сравнительная производительность или удобство. Тесты границы, отказа, порядка принятия и возврата проверяют поведение и не меняются при рефакторинге. Типы и wiring примера — доступный для проверки исходный контракт; второй manifest или таблица маршрутов не поддерживаются. Инструмент может получить такие представления, если они нужны проекту. Примеры подтверждают эту ограниченную форму на этих сценариях; они не доказывают удобства для всех систем или начинающих людей. [Оценка для проекта](EVALUATION.md) описывает более широкое измеряемое сравнение.
