...
| 2 | Конечные семантические пределы и пределы среды исполнения | §§8.3–8.4, 10.9, 13.1–13.2; PBA-38 | `SECURITY-LIMITS-RUNBOOK` для Decision Work Meter при применимости; `COMPOSITION-PROFILES` для пределов графа; `ASYNC-STATUS-RUNBOOK` для допуска только по ёмкости, резервов альтернативного завершения и статуса |
...
| 4 | Предварительная проверка, резервирование и допуск | §§8.4, 8.7, 13.2 | `DESIGN-RUNBOOK`, `SECURITY-LIMITS-RUNBOOK`; `ASYNC-STATUS-RUNBOOK` для передачи альтернативного завершения уровня 1, результата уровня 2, причинных резервов и резервов статуса |
...
| 8 | Вход команды, возврат принятого результата и отказ до принятия | §§6.8–6.13, 8.4–8.9, 9.1–9.4, 10.2/10.7/10.11; PBA-18/PBA-19 | `ASYNC-STATUS-RUNBOOK`, `MANIFEST-AND-ASSEMBLY`, `DESIGN-RUNBOOK`; точный мост, класс отказа, кортеж результата, выбор команды или чтения, передача альтернативного завершения уровня 1, резерв результата уровня 2 и сохранение асинхронного бюджета |
