...
Манифест прямо объявляет каждую непустую собственную поверхность Checkout из примера: изменяющие намерения `CheckoutStarted` и `CancellationRequested`, доверенный управляющий вход после фиксации `CheckoutCommandDeliveryObserved`, сопоставление запроса и результата `GetCheckoutStatus -> CheckoutStatus` и зафиксированную полезную нагрузку ответа `RequestAccepted`. `CheckoutStatus` — единственная замкнутая полезная нагрузка в §16.13 для известной, отсутствующей операции или операции с истёкшим сроком хранения; ни транспортный 404, ни второй тип результата её не заменяют. Отмена покупателем проходит через Interaction и не объявляется как `ControlPulse`; `CheckoutCommandDeliveryObserved` принимается только от объявленного доверенного runtime или источника маршрута после проверки дескриптора и происхождения. Четыре строки `flowParticipations` разрешают Cart, Inventory, Payment и Order ровно по одному разу относительно `Checkout.ownedCoordination`; Auth — вспомогательный владелец контекста, а Checkout — владелец Flow, поэтому ни один не становится строкой участника. Сопоставления команд/результатов участников и статические классификации отказов по-прежнему импортируются ровно через девять версионированных записей `dependencies.commands`, а не копируются в `spec.protocols` или строки участия; это сохраняет владение получателя и запрет реэкспорта протокола. Поэтому по §10.9 Checkout владеет тринадцатью учитываемыми объявлениями: четырьмя различными строками `FlowParticipation` и девятью различными строками `DeclaredCommandDependency`. Ссылка участия не стирает объявление команды, на которое ссылается, и не создаёт третью копию. Соответствующий перечень маршрутов §14.4 содержит ровно девять действующих строк полного пути команды туда и обратно; каждая строка связывает проверенный вход `ModuleCommandPulse` и возврат `ModuleResultPulse` из принятого фрейма для одного принадлежащего получателю сопоставления и считается один раз, а не два.

Точная политика `workflow-durable` задаёт неизменные общие контракты профиля, входа, повторов, доставки, возможностей и базовых пределов. Checkout записывает только разрешённые отличия своего процесса. Показанное `maxTransitionSteps: 4096` полно только потому, что та же точная политика разрешает одну неизменяемую идентичность/версию `DecisionWorkMeter`, версию артефакта перехода, определение единицы и политику превышения по §8.3; числовое отличие не может молча заменить счётчик. Та же политика задаёт действующие числовые `maxStateBytes` и `maxOutputBytesPerDecision` Checkout через неизменяемые кортежи `BoundedByteMeasure` для каждого измерения либо замыкает любое из этих измерений статическим доказательством ограниченного типа и представления; локальный манифест не копирует общие контракты. Замкнутая алгебра выходов Checkout, граф из девяти маршрутов, предел выходов на Decision и предел причинной глубины дают статическое доказательство §10.9 для действующего `maxCumulativeFanout`, поэтому локальный счётчик или дублирующая числовая строка не показаны; привязка, которая не может сохранить это точное статическое доказательство, должна вместо этого разрешить и обеспечить тот же совокупный предел ветвей через существующее причинное резервирование. Замкнутый протокол не содержит частного `EffectRequest`, поэтому нулевой предел эффектов или строка `not-applicable` не нужны: вся работа выполняется через объявленные команды участников. Доменный `maxRetainedDispatchStops: 10` покрывает начальный `ReplyOutput` `RequestAccepted` и девять замкнутых слотов шагов команд Checkout из §16.3, не превышает действующий `maxCollectionItems` и не становится новым обязательным пределом Core. `stateSchemaVersion: 2` фиксирует сохранённые значения процесса, добавленные в §16.3; собственные и импортированные варианты протокола, версии зависимостей и маршруты Assembly не меняются. Миграция уже хранимого состояния остаётся отдельным контрактом развёртывания или расширения по §§8.9/10.11: активное состояние v1 нельзя молча дополнять null или значениями M/S/I/P по умолчанию; привязка либо восстанавливает их из объявленных авторитетных доказательств миграции, либо запрещает обычный `decide` v2 и переводит операцию на объявленный путь карантина или ручной обработки.
