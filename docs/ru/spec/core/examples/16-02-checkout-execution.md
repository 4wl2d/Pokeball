...
Следующий сериализованный Decision Checkout потребляет этот Pulse. Cart принимает собственную работу до возврата принятого результата; отказ до принятия не создаёт принятого фрейма Cart и поступает в Checkout через объявленное наблюдение отказа. Здесь Checkout сохраняет переносимые причинные свидетельства, потому что workflow восстанавливаем и независимо наблюдаем, а не потому, что маршрут пересекает Ball. Реальные пределы сохраняемой работы и вместимости завершений остаются в силе без обязательного резервирования уровней 1/2. Только прямой вызов создаёт `Checkout -> Cart`; ни одна ветвь возврата не создаёт `Cart -> Checkout`. Немедленному примеру Counter в руководстве по композиции нужны лишь типизированный получатель, принятая отправка и порядок вызова/возврата, без восстановления отсутствующих причинных кортежей.
