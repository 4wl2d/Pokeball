...
В маршруте участника в том же стеке попытка цели, отклонённая до принятия, не расходует принятый уровень цели. Decision `ControlPulse` Checkout, который фиксирует эту проекцию носителя, расходует переданный исходной командой слот альтернативного завершения уровня 1 согласно §8.4; он не откладывается в новый корень и не сбрасывает общий причинный бюджет.
