...
В Core есть 44 канонических закона. Таблица помогает найти нужную группу, но не заменяет точный текст. Это не означает 44 локальных артефакта: §20.1 задаёт для каждого закона класс применимости, точный триггер, владельца декларации, владельца enforcement или evidence и правило повторного использования.
