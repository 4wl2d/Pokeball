...
- immediate same-build command tests verify typed target access, source acceptance before dispatch, target-owned acceptance, and serialized source completion; they require no reconstructed token or issuer tuple. Independently delivered commands additionally verify the required portable source/result identity and provenance;
...
- closed local command fixtures finish after source acceptance, one target attempt and source completion without carried causal levels or reservation slots; target nonacceptance leaves its state unchanged, and post-acceptance executor failure never becomes nonacceptance. A completion that reissues work invalidates the finite-structure premise even when imports form a DAG. Growing/queued paths reject before accepting work that cannot be retained or completed; handoff cannot reset their causal bounds;
