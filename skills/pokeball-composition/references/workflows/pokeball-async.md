...
Operate on the consuming project. Resolve supporting references within this skill's directory. Read [working contract](../working-contract.md) once per task and select the relevant [async routes](../async.md).
...
Return the code/test delta and what the observed trace now does. State any unresolved external-outcome or binding assumption. Detached execution alone does not select durable storage; crash survival additionally needs the actual [binding workflow](pokeball-binding.md).
