...
Work on the consuming project. Resolve supporting references within this skill's directory. Read [working contract](../working-contract.md) once and select the applicable [composition routes](../composition.md).
...
Report the ownership decision, changed contracts/edges and evidence. Add a diagram only when it clarifies the changed graph; source may already carry the map. Command execution details use [async](pokeball-async.md); changing the enforcement machinery uses [binding](pokeball-binding.md).
